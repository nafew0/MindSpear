...
const SortAnswerBlanksComponent: React.FC<SortAnswerBlanksProps> = ({ 
  ...
  const storageKey = `quiz-${question.quiz_id}-q${id}-items`;
      ...
      if (savedItems) {
        try {
          ...
          if (Array.isArray(parsedItems)) {
            ...
            if (onAnswerChange) {
              onAnswerChange(parsedItems.length > 0);
            }
          }
        } catch (e) {
  ...
  useEffect(() => {
    if (hasHydrated && typeof window !== 'undefined') {
      localStorage.setItem(storageKey, JSON.stringify(additionalItems));
    }
    if (onAnswerChange) {
      onAnswerChange(additionalItems.length > 0 || newItemText.trim().length > 0);
    }
  }, [additionalItems, newItemText, storageKey, onAnswerChange, hasHydrated]);
            ...
            <div className="flex items-center gap-2 relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (onAnswerChange) {
                    onAnswerChange(e.target.value.trim().length > 0 || additionalItems.length > 0);
                  }
                }}
