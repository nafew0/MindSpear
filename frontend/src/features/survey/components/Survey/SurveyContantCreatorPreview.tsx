...
const SurveyContantCreatorPreview: React.FC<{ id: string }> = ({ id }) => {
	...
	const handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
		if (currentItem) {
			actions.contantData({
				id: currentItem.id,
				contant_title: e.target.value,
				image_url: currentItem.image_url,
				layout_id: currentItem.layout_id,
			});
		}
	};
		...
		<div className="grid grid-cols-1 md:grid-cols-12 gap-5">
			<div className="md:col-span-9">
				<div className="flex-1 p-4 bg-white border-2 border-[#bc5eb3] rounded-[10px] h-[calc(90vh-80px)] flex flex-col justify-start overflow-y-auto scrollbar-hidden">
					<div className="mb-4">
						<textarea
							value={currentItem?.contant_title || ""}
			...
			<div className="md:col-span-3 p-4 bg-white rounded-lg h-[calc(90vh-50px)] overflow-y-auto scrollbar-hidden">
				...
				<div className="bg-gray-50 p-4 rounded-md">
					...
					<p className="text-sm text-gray-600">Information/Content</p>
				</div>
			</div>
		</div>
	);
...
export default SurveyContantCreatorPreview;
