	...
	const dataFetch = async () => {
		try {
			...
			const responseData = await axiosInstance.get(
				`/quizes?page=${currentPage}&order_by_id=desc&per_page=${pageSize}`,
			);
			setdatalist(responseData?.data?.data?.quizes?.data);
			setTotal(responseData?.data?.data?.quizes?.total || 0);
					...
					>
						<div className="p-5">
							...
							<div
								...
								data-tooltip-content="Delete Quiz"
