...
function QuestionsResultView({ list }: any) {
	...
	const filteredAndSortedSessions = useMemo(() => {
		// Show only ended sessions (running_status === false), then apply search
		const filtered = (list || [])
			.filter((session: any) => session?.running_status === false)
			.filter((session: any) =>
				session?.title
					?.toLowerCase()
	...
	const downloadCsv = async (data: any) => {
		try {
			...
			const response = await axiosInstance.get(
				`/quest-leaderboard/download-session-attempts/${data?.id}/export/csv`,
				{ responseType: "blob" },
			);
	...
	const downloadExcel = async (data: any) => {
		try {
			...
			setActiveId(data?.id);
			const response = await axiosInstance.get(
				`/quest-leaderboard/download-session-attempts/${data?.id}/export/excel`,
				{ responseType: "blob" },
			);
...
export default QuestionsResultView;
