...
const resolveByIdOrIndex = <T extends { id: number }>(v: number, arr: T[]) =>
	(Number.isInteger(v) && v >= 0 && v < arr.length ? arr[v] : undefined) ??
	arr.find((x) => x.id === v);
...
function buildViewProps(sub: Submission): ViewProps {
	...
	if (
		...
		const n = asNumber(selected);
		if (Number.isFinite(n)) {
			const resolved = resolveByIdOrIndex(n as number, questions);
...
function ChartPanel({ sub }: { sub: Submission }) {
	const {
		...
		rankinData,
		values,
	} = useMemo(() => buildViewProps(sub), [sub]);
	const scaleItems = transformTaskData(sub);

	console.log(sub, "currentViewcurrentViewcurrentViewcurrentViewcurrentView");
	console.log(
		data,
			...
			currentView === "fill_in_the_blanks_choice" ? (
				...
				<div className="w-full px-4 h-full">
					<ScalesChart
						title=""
						items={scaleItems}
						full={sub?.task?.task_data?.maxNumber}
					/>
				</div>
			) : currentView === "ranking" ? (
...
export default function SingleResultView({
	...
	console.log(resultList, "resultList999");
	...
	return (
		<div
			className={cn(
				"w-full p-4 lg:p-10 relative min-h-screen",
				className
			)}
		>
							...
							return (
								...
								>
									...
									<div className="flex flex-col justify-center items-center ">
										...
										<span className="text-xs lg:text-sm pt-0 font-bold">
											{/* {t.label} */}
