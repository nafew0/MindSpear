...
## Production Update (Ubuntu Server)

- Frontend: `https://mindspear.app`
- API: `https://api.mindspear.app`
...
Use before a clean restart or if a service is stuck.

```bash
sudo systemctl stop mindspear-reverb mindspear-queue
pm2 stop mindspear-frontend
```

### Start services

Use if any service is down.

```bash
sudo systemctl start mindspear-reverb mindspear-queue
pm2 start mindspear-frontend
```
...
```bash
php /var/www/mindspear/backend/artisan queue:restart
sudo systemctl restart mindspear-reverb mindspear-queue
sudo systemctl reload php8.4-fpm
...
### Check that backend + frontend services are running

Use after deploy or restart.

```bash
sudo systemctl status php8.4-fpm redis-server postgresql nginx mindspear-reverb mindspear-queue --no-pager
pm2 status mindspear-frontend
curl -I https://mindspear.app
curl -I https://api.mindspear.app/api/v1/health || true
```
